...
public class RestResponseEntityExceptionHandler
  ...
  protected ResponseEntity<Object> handleConflict(
    ...
    ArrayList<String> errors = ex.getConstraintViolations().stream()//
        .map(error -> "Field '" + error.getPropertyPath().toString() + "' " + error.getMessage()).collect(Collectors.toCollection(ArrayList::new));
    return ResponseHandler.generateResponse(new ApiException(-1, "Bad Input"), HttpStatus.BAD_REQUEST, errors);
  }
